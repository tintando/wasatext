...
func run() error {
	...
	logger.Infof("database file: %s", cfg.DB.Filename)

	// Ensure database directory exists
	dbDir := filepath.Dir(cfg.DB.Filename)
	if dbDir != "." && dbDir != "/" {
		logger.Infof("creating database directory: %s", dbDir)
	...
	logger.Infof("creating photo directories: %s", cfg.Photos.BasePath)
	err = os.MkdirAll(cfg.Photos.UsersPath, 0755)
	if err != nil {
		logger.WithError(err).Warning("error creating users photo directory")
	}
	err = os.MkdirAll(cfg.Photos.GroupsPath, 0755)
	if err != nil {
		logger.WithError(err).Warning("error creating groups photo directory")
	}
	err = os.MkdirAll(cfg.Photos.MessagesPath, 0755)
	if err != nil {
		logger.WithError(err).Warning("error creating messages photo directory")
	}
	logger.Info("photo directories created successfully")

	// Create the API router
	apirouter, err := api.New(api.Config{
		Logger:   logger,
		Database: db,
		Photos: api.PhotoConfig{
			UsersPath:    cfg.Photos.UsersPath,
			GroupsPath:   cfg.Photos.GroupsPath,
			MessagesPath: cfg.Photos.MessagesPath,
		},
	})
	if err != nil {
